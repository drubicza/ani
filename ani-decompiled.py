...
def getimage(link):
	...
	if len(cek) != 0:
			...
			names.append(ui+'_'+pref+'.jpg')
			content.append(dl.get('href'))
	print(f'\rget {len(content)} images..',end=''),;sys.stdout.flush()
...
def getvideo(link):
	...
	if len(cek) != 0:
		for div in cek:
			dl = div.find('a',attrs={'class':'btn btn-primary'})
			pref = re.findall(r'\/vp/(.*?)\/',str(div))[0][:5]
...
def menu():
	banner()
	print(f"{lc}Type {lx}'help'{lc} to show the options")
	sss = input(f'{lc}\n>> {lx}')
	if sss == 'start':
		banner()
		try:
